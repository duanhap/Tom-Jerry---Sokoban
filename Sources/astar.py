...
def AStart_Search(board, list_check_point):
    TIME_OUT = 15.0  # Đặt thời gian tối đa 5 giây
    start_time = time.time()
    
    # Kiểm tra trạng thái thắng ban đầu
    if spf.check_win(board, list_check_point):
        print("Found win")
        return [board]
    ...
    list_state = set()  # Lưu trữ trạng thái đã duyệt
    heuristic_queue = PriorityQueue()
    heuristic_queue.put(start_state)
    states_explored = 0  # Đếm số trạng thái đã duyệt
    ...
    while not heuristic_queue.empty():
        ...
        # Tạo và kiểm tra các trạng thái mới
        for next_pos in list_can_move:
            new_board = spf.move(now_state.board, next_pos, cur_pos, list_check_point)
            
            # Chuyển board thành tuple để lưu vào set
            board_tuple = tuple(map(tuple, new_board))
            
            # Kiểm tra trạng thái hợp lệ
            if (board_tuple in list_state or
                    spf.is_board_can_not_win(new_board, list_check_point) or
                    spf.is_all_boxes_stuck(new_board, list_check_point)):
                continue
            ...
            if spf.check_win(new_board, list_check_point):
                print("Found win")
                print(f"Số trạng thái đã duyệt: {states_explored}")
                return (new_state.get_line(), states_explored)
            ...
            if time.time() - start_time > TIME_OUT:
                print("Timeout after", TIME_OUT, "seconds")
                return []
        
        # Giới hạn số trạng thái để tránh quá tải bộ nhớ
        if states_explored > 100000:  # Giới hạn 100,000 trạng thái
            print("Reached state exploration limit")
            return []
